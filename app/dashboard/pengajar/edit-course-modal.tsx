...
export default function EditCourseModal({ courseId, currentJudul, currentDeskripsi }: Props) {
  ...
  return (
    <>
      <button
        ...
        className="px-3 py-1.5 text-xs font-medium rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition"
      >
        ✏️ Edit
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md p-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Edit Kelas</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Judul Kelas</label>
              ...
              <div>
                ...
                <textarea value={deskripsi} onChange={(e) => setDeskripsi(e.target.value)} required rows={4}
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setOpen(false); setError(""); setJudul(currentJudul); setDeskripsi(currentDeskripsi); }}
